...
import pandas as pd
from utils import load_yaml, set_seeds
from src.modeling.finetune_utils import Finetuner
import logging


def read_yaml(file, model_save_dir='', multiple=False, source_model='source_9_multi'):
    ...
    # Load the source model that was saved in the ´experiments´ or 'model_source' directory.
    #Georgia: split_1_1, CPSC: 2_1, Chapman: 3_1, PTB: 4_1, SPH: 5_1 source models

    source_model_path = os.path.join(os.getcwd(), 'model_source', source_model)
    args.load_model_path = os.path.join(source_model_path, args.model)

    # Check if model_path never set, i.e., the trained model was not found
    if not hasattr(args, 'load_model_path'):
        raise AttributeError('No path found for the model. Check if you have trained one.')
    else:
        print("Source model path: {}".format(args.load_model_path))
    ...
    args.logger.info('Finetuning a model...')
    finetuner = Finetuner(args)
    finetuner.setup()
    finetuner.finetune()


def read_multiple_yamls(path, source_model):
    """ Read multiple yaml files from the given directory

    :param path: Absolute path for the directory
    :type path: str
    """
    # All yaml files
    ...
    for file in yaml_files:
        read_yaml(file, model_save_dir, True, source_model)
...
if __name__ == '__main__':
    ...
    arg_path = os.path.join(os.getcwd(), 'configs', 'finetuning', given_arg)
    print("YAML path: ", arg_path)
    ...
    if os.path.exists(arg_path):
        ...
        if given_arg.lower().endswith('.yaml'):  # Modified to endswith to allow folders named with yaml
            # Run one yaml
            print("File: ", arg_path)
            read_yaml(arg_path, source_model)
        else:
            # Run multiple yamls from a directory
            read_multiple_yamls(arg_path, source_model)
